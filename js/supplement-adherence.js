...
export function renderSupplementAdherence(logs, endIso) {
  ...
  const stats = computeAdherenceStats(byDate, endIso, ADHERENCE_DAYS, profile);
  ...
  const showsD3 = (profile.supplementKeys || []).includes('uprise_d3_60k');
  ...
  el.innerHTML = `
    <div class="supp-adherence-stats">
      ...
      ${showsD3 ? `<div class="supp-adherence-stat">
        <strong>${stats.d3Pct != null ? `${stats.d3Pct}%` : '—'}</strong>
        <span>Weekly D3 (${stats.d3Taken}/${stats.d3Required})</span>
      </div>` : ''}
    </div>
    <p class="form-hint">${formatSupplementAdherenceHint(profile)}</p>
    ${showsD3 ? `<p class="form-label mt-12">D3 dose history</p>${d3List}` : ''}
  `;
}
